    ...
    local cmp_mapping_config = {
      ...
      ['<Tab>'] = cmp.mapping({
        c = function()
          ...
          if cmp.visible() then
            -- print('c-next')
            cmp.select_next_item({ behavior = cmp.SelectBehavior.Inser })
          else
            -- print('c-complete')
            cmp.complete()
          end
      ...
      ['<S-Tab>'] = cmp.mapping({
        c = function()
          ...
          if cmp.visible() then
            print('c-prev')
            cmp.select_prev_item({ behavior = cmp.SelectBehavior.Inser })
          else
            print('c-complete')
            cmp.complete()
          end
    ...
    cmp.setup({
      ...
      formatting = {
        ...
        format = function(entry, vim_item)
          ...
          print(string.format("%s - %s %s", entry.source.name, vim_item.kind, lspkind.symbolic(vim_item.kind, { mode = 'symbol_text' })))
          local menu = ({
                buffer = '[Buf]',
                nvim_lsp = '[LSP]',
                nvim_lua = '[Lua]',
...
--   elseif cmp_helper.is_prior_char_whitespace() then
